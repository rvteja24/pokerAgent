...
class InformationAbstracter:
    ...
    def get_tree_name(self, cardPair):
        card1 = cardPair[0]
        card2 = cardPair[1]
        if card1[0] == card2[0]:
            if card1[0] in self.pairsTreeDict.keys():
                ...
                self.pairsTreeDict[card1[0]] = "strategies/pairsTrees/pairsTree" + card1[0] + ".txt"
                with open('treeDicts/pairsTreeDict.txt', 'wb') as file:
                    file.write(pickle.dumps(self.pairsTreeDict))
                return "strategies/pairsTrees/pairsTree" + card1[0] + ".txt"
            ...
            if (card1[0], card2[0]) in self.sameSuitTreeDict.keys():
                ...
                self.sameSuitTreeDict[(card1[0], card2[0])] = "strategies/sameSuitTrees/sameSuitTree" + card1[0] + \
                                                              card2[0] + ".txt"
                with open('treeDicts/sameSuitTreeDict.txt', 'wb') as file:
                    file.write(pickle.dumps(self.sameSuitTreeDict))
                return "strategies/sameSuitTrees/sameSuitTree" + card1[0] + card2[0] + ".txt"
        else:
            if (card1[0], card2[0]) in self.trashTreeDict.keys():
                ...
                self.trashTreeDict[(card1[0], card2[0])] = "strategies/trashTrees/trashTree" + card1[0] + card2[
                    0] + ".txt"
                with open('treeDicts/trashTreeDict.txt', 'wb') as file:
                    file.write(pickle.dumps(self.trashTreeDict))
                return "strategies/trashTrees/trashTree" + card1[0] + card2[0] + ".txt"

    # with the tree name the tree is read and returned
    def get_tree_details(self, cardPair):
        tree_name = self.get_tree_name(cardPair)
        tree = []
